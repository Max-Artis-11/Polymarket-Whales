  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (version === 'v5') {
        ...
        // ─── V6: sequential with scoring ───
        for (let i = 0; i < all.length; i++) {
          const w = all[i];
          setProgressMsg(`Scoring ${i + 1} / ${all.length} — ${w.name}`);
          ...
          const walletScore = scoreWallet(positions, w.profit, w.volume);

          if (walletScore && filters.accountSize !== 'all') {
            const size = getAccountSize(w.volume);
            if (size !== filters.accountSize) {
              setProgressPct(8 + ((i + 1) / all.length) * 84);
              await sleep(80);
              continue;
            }
          ...
          if (walletScore && walletScore.totalPositions >= 5 && walletScore.realWinRate < 30) {
            setProgressPct(8 + ((i + 1) / all.length) * 84);
            await sleep(80);
            continue;
          }
