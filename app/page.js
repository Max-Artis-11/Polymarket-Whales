...
export default function Home() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (version === 'v5') {
        // ─── V5: fire all position fetches in parallel batches of 100 ───
        const BATCH = 100;
        for (let b = 0; b < all.length; b += BATCH) {
          ...
          for (const w of results) {
            ...
            const filtered = w.positions.filter((p) =>
              filterPosition(p, { minProb: filters.minProb, maxProb: filters.maxProb, category: filters.category })
            );
    ...
    <div style={S.page}>
      ...
      <div style={S.filterBar}>
        ...
        <div style={S.fieldGroup}>
          ...
          <NumInput
            value={filters.leaderboardLimit} min={0} max={1000} width={64}
