...
export function scoreWallet(positions, walletProfit = 0, walletVolume = 0) {
  ...
  for (const p of positions) {
    ...
    if (isCryptoTrade(title)) cryptoCount++;

    const avgPrice = p.avgPrice ?? 0;
    if (avgPrice > 0 && avgPrice < 1) {
      entryProbSum += avgPrice;
      entryProbCount++;
    }

    if (p.redeemable || curPrice >= 0.99 || curPrice <= 0.01) {
      if (cashPnl > 0 || currentValue > initialValue * 1.1) wins++;
      else losses++;
      continue;
    }

    if (initialValue > 0 && currentValue < initialValue * 0.1) {
      dead++;
      losses++;
      continue;
    }

    if (cashPnl > 0 || currentValue > initialValue) wins++;
    else losses++;
  }
  ...
  if (walletVolume > 0 && walletVolume < 50000 && realWinRate > 0.55) {
    score *= 1.2;
  }
  ...
  return {
    ...
    realWinRate: Math.round(realWinRate * 100),
    avgEntryProb: Math.round(avgEntryProb * 100),
    isCryptoHeavy: cryptoRatio > 0.5,
    deadPositions: dead,
    totalPositions: total,
    wins,
    losses,
    cryptoRatio: Math.round(cryptoRatio * 100),
    volume: walletVolume,
  };
}
...
const SPORTS_LEAGUES = /\b(nba|nfl|mlb|nhl|mls|epl|la liga|serie a|bundesliga|ligue 1|atp|wta|ufc|mma|nascar|f1|formula|cricket|wnba|kbo|npb|lol|dota|counter-strike|csgo|cs2|valorant|overwatch|league of legends|esports?|e-sports?)\b/i;
...
export function filterPosition(pos, { minProb = 15, maxProb = 85, category = 'all' } = {}) {
  ...
  for (const { address, name, positions, walletScore } of walletDataList) {
    for (const pos of positions) {
      ...
      if (!map[key].wallets.find((w) => w.address === address)) {
        map[key].wallets.push({ address, name, score: walletScore });
      }
    }
  }
